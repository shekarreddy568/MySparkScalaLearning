...
object TitanicDataAnalysis {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val df = data
      .map(y => y.split(","))
      .map(x => (x(1).asInstanceOf[String], x(4).asInstanceOf[String], x(5).asInstanceOf[String]))
      .toDF("Survived", "Sex", "Age")

    val res = df
      .filter($"Age" !== "")
      .filter($"Survived" !== "")
      .filter($"Survived" === "1")
      .select($"Sex", $"Age".cast("Int"))
      .groupBy("Sex")
      .avg("Age")
    ...
    val res2 = df2
      .filter($"Age" !== "")
      .filter($"Survived" !== "")
      .select($"Survived", $"Sex", $"Age", $"Pclass".cast("String"))
      .groupBy("Survived", "Sex", "Age", "Pclass")
      .count()
    res2.show()
  }
